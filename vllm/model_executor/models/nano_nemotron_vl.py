...
class NemotronH_Nano_VL_V2(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Dispatch vision encoder by `vision_model_type` from config.json (set
        # by `examples/multimodal/tools/create_yaml_inference_config.py::HF_OVERRIDES`
        # at mcore→HF conversion time). Defaults to "radio" for backwards
        # compatibility with checkpoints that predate the field.
        self.vision_model_type = getattr(config, "vision_model_type", "radio")
        with self._mark_tower_model(vllm_config, {"image", "video", "audio"}):
            if self.vision_model_type == "radio":
                self.vision_model = self.get_vit_model_from_radio_config(config).to(
                    llm_dtype
                )
                # RADIO emits raw patch features; the post-encoder spatial
                # reduction (pixel_shuffle) happens downstream in this model.
                self._encoder_did_spatial_merge = False
            elif self.vision_model_type == "pixtral-vit-large":
                ...
                raise ValueError(
                    f"unsupported vision_model_type: {self.vision_model_type!r}; "
                    f"expected 'radio' or 'pixtral-vit-large'"
                )

            # Construct the vision projection.
            # For Pixtral, vit_hidden_size comes from the adapter's encoder
            # rather than `config.vit_hidden_size` (which the rsync'd template
            # config defaults to a RADIO value of 1280). Same for the projector
            # input dim — Pixtral's internal merger has already done the 4×
            # spatial reduction, so projector fan_in = encoder hidden (not 4×).
            if self.vision_model_type == "pixtral-vit-large":
            ...
            # conversion.
            # Projector fan_in. For RADIO this is post-pixel-shuffle width
            # (encoder hidden × 4 for ds_ratio=0.5). For Pixtral, the native
            # merger already gave a 4× reduction and pixel-shuffle is skipped,
            # so projector fan_in = encoder hidden directly.
            if self._encoder_did_spatial_merge:
                projector_input_size = vit_hidden_size
            else:
    ...
    def extract_feature_dynamic(
        ...
        # Pixtral's native merger inside the encoder adapter has already done
        # the 4× spatial reduction (RMSNorm + Linear(4H→H)); applying
        # pixel_shuffle on top would double-reduce. Skip when the encoder
        # already merged. RADIO (no internal merger) still uses the downstream
        # pixel_shuffle path.
        if not self._encoder_did_spatial_merge:
            vit_embeds = self.pixel_shuffle_dynamic_res(
                vit_embeds, imgs_sizes=imgs_sizes
            )
    ...
    def extract_feature(
        ...
        # For Pixtral, the encoder adapter ALREADY runs the native merger
        # (RMSNorm + Linear(4H→H)) and returns post-merger features per image.
        # Downstream pixel_shuffle must be skipped. Conv3d/video-tubelet
        # encoding isn't implemented in mcore for Pixtral (Step 10 of plan).
        if self._encoder_did_spatial_merge and num_frames is not None \
                and self.video_temporal_patch_size > 1:
            raise NotImplementedError(
                "Pixtral conv3d video encoding (T > 1) is not implemented in "
                "mcore — train and convert with `--video-temporal-patch-size=1` "
                "until Step 10 of the vision-encoder-study plan lands."
            )
        ...
        for i in range(0, N, micro_batch_size):
            ...
            vit_embeds = vit_embeds.to(dtype=torch.bfloat16)
            if not self._encoder_did_spatial_merge:
                # RADIO path: reshape to grid + pixel_shuffle for 4× reduction.
            ...
            # Pixtral path: vit_embeds is already (chunk_size, n_post_merger, H)
            # from the adapter's 4-D forward; no reshape/shuffle needed.
            vit_embeds = self.mlp1(vit_embeds)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]):
        ...
        def is_vision_weights(name: str) -> bool:
            # mcore→HF saver writes RADIO under `vision_model.radio_model.*`
            # and Pixtral under `vision_model.vision_model.*` (see
            # `tools/checkpoint/saver_hf_moe_llava.py::receive_model`).
            # Generalize to all `vision_model.*` keys; routing to the right
            # encoder load_weights happens via `self.vision_model.load_weights`.
